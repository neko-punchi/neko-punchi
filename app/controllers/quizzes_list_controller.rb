...
require 'pp'
  ...
  def edit
    ...
    @quiz_data = Relation.find_by_sql(["select DD.id as category_id,DD.category,BB.id as sub_category_id,BB.sub_category, CC.* from relations AA inner join sub_categories BB on AA.sub_category_id = BB.id inner join quizzes CC on AA.quiz_id = CC.id inner join categories DD on AA.category_id = DD.id where CC.id = :quiz_id", :quiz_id => quiz_id])
